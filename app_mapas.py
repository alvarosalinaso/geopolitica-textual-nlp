...
st.set_page_config(
    page_title="Cartografía Textual Histórica", layout="wide", page_icon="🗺️"
)

st.title("🗺️ Geopolítica Textual: Mapas HD")
st.markdown(
    "Plataforma interactiva de **Distant Reading**. Descubre qué territorios acaparan el foco político leyendo automáticamente cientos de actas de gobierno con IA."
)
...
if df_places.empty:
    st.error(
        "No se pudo iniciar el modelo NLP. Asegúrate de haber instalado 'es_core_news_sm'."
    )
    ...
    with col1:
        ...
        selected_years = st.multiselect(
            "Seleccionar Siglo / Década", sorted(years), default=sorted(years)
        )
    ...
    st.divider()
    st.caption(
        "Arquitectura NLP base (spaCy) · Motor Gráfico: Folium / OpenStreetMaps · Proyecto de Humanidades Digitales"
    )
